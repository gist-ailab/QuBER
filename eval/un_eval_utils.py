...
from base_model import UOAISNet, UCN, UOISNet, MSMFormer, SAM, LoadNpyBaseModel, Empty, GT, Detic
from refiner_model import MaskRefiner, RICE, CascadePSP, LoadNpyRefinerModel
...
def run_eval(args):
    ...
    if args.base_model == "uoaisnet":
        ...
        base_model = SAM()
    elif args.base_model == "sam-prompt":
        base_model = SAM(prompt='all objects on the table or floor')
    elif args.base_model == "npy":
        base_model = LoadNpyBaseModel(npy_folder='/SSDe/seunghyeok_back/mask-refiner/segfix/{}/inital_mask_predict'.format(args.test_dataset))
    ...
    if args.refiner_model == "maskrefiner":
        ...
        refiner_model = LoadNpyRefinerModel(npy_folder='/SSDe/seunghyeok_back/mask-refiner/segfix/{}/label_w_segfix_detic_20_0.6'.format(args.test_dataset, args.dt_max_distance, args.mask_threshold),
                                            dataset=args.test_dataset)
    elif args.refiner_model == "custom":
        refiner_model = MaskRefiner(args.config_file, 
                                weights_file = args.weights_file,
                                dataset=args.test_dataset)
        refiner_model_rice = RICE(base_model=args.base_model, 
                             dataset=args.test_dataset,
                            )
        refiner_model_cascadepsp = CascadePSP(L=900, fast=False, dataset=args.test_dataset)
    else:
    ...
    if args.test_dataset == "unstructured_test":
        args.dataset_path = 'detectron2_datasets/unstructured_test'
        ...
        # wrong_anno_paths = ["learn25.png", "learn26.png", "test25.png", "test26.png", "test49.png"]
        # wrong_anno_paths = ["test25.png", "test26.png", "test49.png", "test64.png", "test63.png", "test62.png", "test60.png", "test58.png", "test55.png", "test54.png", "test53.png", "test52.png", "test51.png", "test44.png",
        #                     "learn0.png", "learn17.png", "learn18.png", "learn19.png", "learn21.png", "learn25.png", "learn26.png", "learn28.png", "learn29.png", "learn30.png", "learn31.png", "learn32.png"]
        # for wrong_anno_path in wrong_anno_paths:
        #     # os.remove("{}/annotation/{}".format(args.dataset_path, wrong_anno_path))
        #     try:
        #         os.remove("{}/disparity/{}".format(args.dataset_path, wrong_anno_path))
        #     except:
        #         pass
        #     try:
        #         os.remove("{}/image_color/{}".format(args.dataset_path, wrong_anno_path))
        #     except:
        #         pass

            # idx = anno_paths.index("{}/annotation/{}".format(args.dataset_path, wrong_anno_path))
            # rgb_paths.pop(idx)
            # depth_paths.pop(idx)
            # anno_paths.pop(idx)
    ...
    for rgb_path, depth_path, anno_path in zip(tqdm(rgb_paths), depth_paths, anno_paths):
        ...
        # refined_masks, refined_output, refined_pred_time, fg_mask = refiner_model_rice.predict(rgb_path, depth_path, refined_masks, fg_mask)
        refined_masks, refined_output, refined_pred_time, fg_mask = refiner_model_cascadepsp.predict(rgb_path, depth_path, refined_masks, fg_mask)
        # refined_masks, refined_output, refined_pred_time, fg_mask = refiner_model_cascadepsp.predict(rgb_path, depth_path, refined_masks, fg_mask)
        # refined_masks, refined_output, refined_pred_time, fg_mask = refiner_model_cascadepsp.predict(rgb_path, depth_path, refined_masks, fg_mask)
        # refined_masks, refined_output, refined_pred_time, fg_mask = refiner_model_rice.predict(rgb_path, depth_path, refined_masks, fg_mask)
        # refined_masks, refined_output, refined_pred_time, fg_mask = refiner_model_cascadepsp.predict(rgb_path, depth_path, refined_masks, fg_mask)
        # refined_masks, refined_output, refined_pred_time, fg_mask = refiner_model_rice.predict(rgb_path, depth_path, refined_masks, fg_mask)
        # refined_masks, refined_output, refined_pred_time, fg_mask = refiner_model_cascadepsp.predict(rgb_path, depth_path, refined_masks, fg_mask)
        # refined_masks, refined_output, refined_pred_time, fg_mask = refiner_model.predict(rgb_path, depth_path, refined_masks, fg_mask)
        # refined_masks, refined_output, refined_pred_time, fg_mask = refiner_model_rice.predict(rgb_path, depth_path, refined_masks, fg_mask)
        # refined_masks, refined_output, refined_pred_time, fg_mask = refiner_model.predict(rgb_path, depth_path, refined_masks, fg_mask)
    ...
    for i, metrics_all in enumerate([initial_metrics_all, refined_metrics_all]):
        result = {}
        # for idx, metrics in enumerate(metrics_all):
        #     print(idx, rgb_paths[idx], metrics['obj_mIOU_osn'])
        num = len(metrics_all)
