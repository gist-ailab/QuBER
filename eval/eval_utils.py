...
from base_model import SAM, GroundedSAM
from refiner_model import QuBER
...
def run_eval(args):
    ...
    if args.base_model == "sam":
        base_model = SAM(dataset=args.test_dataset)
    elif args.base_model == "grounded-sam":
        base_model = GroundedSAM()
    else:
        raise NotImplementedError("Invalid base model name: {}".format(args.base_model))

    # load refiner model for mask refinement
    if args.refiner_model == "QuBER":
        refiner_model = QuBER(args.config_file, 
                                weights_file = args.weights_file,
                                dataset=args.test_dataset)
    elif args.refiner_model == "sam":
        refiner_model = SAM(prompt_type='mask', dataset=args.test_dataset)
    ...
    if args.test_dataset == "OSD":
        ...
        print("Supported datasets: OSD, OCID, WISDOM")
