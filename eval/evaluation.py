...
def multilabel_metrics(prediction, gt, i, N, 
    ...
    obj_iou = np.zeros((num_labels_gt, num_labels_pred))
    obj_union = np.zeros((num_labels_gt, num_labels_pred))
    ...
    for i, gt_i in enumerate(labels_gt):
        ...
        for j, pred_j in enumerate(labels_pred):
            ...
            ### For unstructured project coupute IOU
            intersection = np.int64(np.count_nonzero(np.logical_and(pred_j_mask, gt_i_mask)))
            union = np.int64(np.count_nonzero(np.logical_or(pred_j_mask, gt_i_mask)))
            obj_iou[i,j] = intersection / union
            obj_union[i,j] = union
    ...
    precision = np.sum(obj_tps[idx]) / np.sum(prediction.clip(0,1) == OBJECTS_LABEL)
    recall = np.sum(obj_tps[idx]) / np.sum(gt.clip(0,1) == OBJECTS_LABEL)
    ...
    obj_iou_osn = np.mean(obj_iou[idx])
    i_over_u = np.sum(obj_tps[idx]) / np.sum(obj_union[idx])
