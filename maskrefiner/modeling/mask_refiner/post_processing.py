...
def get_instance_segmentation(
    center_heatmap, offsets, foreground_seg,
    ...
    Args:
        ...
        offsets: A Tensor of shape [2, H, W] of raw offset output. The order of
            second dim is (offset_y, offset_x).
        mask_error_map: [4, H, W] (TP, TN, FP, FN)
        boundary_error_map: [4, H, W] (TP, TN, FP, FN)
        threshold: A float, threshold applied to center heatmap score.
        nms_kernel: An integer, NMS max pooling kernel size.
        top_k: An integer, top k centers to keep.
    Returns:
    ...
    if center_points.size(0) == 0:
        return torch.zeros_like(foreground_seg), center_points.unsqueeze(0)
    ins_seg = group_pixels(center_points, offsets)
    return foreground_seg * ins_seg, center_points.unsqueeze(0)


def merge_semantic_and_instance(
    ins_seg, foreground_seg, label_divisor, thing_ids, stuff_area, void_label
):
    """
    ...
    pan_seg = torch.zeros_like(foreground_seg) + void_label
    is_thing = (ins_seg > 0) & (foreground_seg > 0)
    ...
    for ins_id in instance_ids:
        ...
        if torch.nonzero(thing_mask).size(0) == 0:
            continue
        if thing_mask.sum().item() < 512:
            continue
        class_id, _ = torch.mode(foreground_seg[thing_mask].view(-1))
    ...
    class_ids = torch.unique(foreground_seg)
    for class_id in class_ids:
        if class_id.item() in thing_ids:
            # thing class
            continue
        # Calculate stuff area.
        stuff_mask = (foreground_seg == class_id) & (ins_seg == 0)
...
def get_panoptic_segmentation(
    ...
    panoptic = merge_semantic_and_instance(
        instance, foreground_seg, label_divisor, thing_ids, stuff_area, void_label
    )
