...
__all__ = ["PerturbedInstanceDatasetMapper"]


class PerturbedInstanceDatasetMapper:
    """
    ...
    def from_config(cls, cfg, is_train=True):
        ...
        if is_train:
            ...
            augs = [T.ResizeShortestEdge(cfg.INPUT.MIN_SIZE_TEST, cfg.INPUT.MAX_SIZE_TEST, "choice")]
    ...
    def __call__(self, dataset_dict):
        ...
        # Load perturbed instance masks
        perturbed_segms = dataset_dict["perturbed_segmentation"]
        perturbed_masks = []
        for segm in perturbed_segms:
            if isinstance(segm, list):
                # polygon
                perturbed_masks.append(polygons_to_bitmask(segm, *image.shape[:2]))
            elif isinstance(segm, dict):
                # COCO RLE
        ...
        if self.size_divisibility > 0 and self.is_train:
            ...
            perturbed_masks = [F.pad(x, padding_size, value=0).contiguous() for x in perturbed_masks]
