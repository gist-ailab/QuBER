...
class PanopticDeepLabTargetGenerator(object):
    ...
    def __call__(self, panoptic, segments_info):
        ...
        for seg in segments_info:
            ...
            if not (self.ignore_crowd_in_semantic):
                # foreground segmentation
                semantic[panoptic == seg["id"]] = 1
                # print(np.unique(semantic), np.unique(panoptic), seg["id"], cat_id)
            if not seg["iscrowd"]:
        ...
        # import imageio
        # print(np.unique(center), np.unique(offset))
        # imageio.imwrite('center.png', center)
        # imageio.imwrite('offset.png', offset[0])
        # imageio.imwrite('offset_.png', offset[1])
        # exit()
...
class PerturbedInputGenerator(object):
    """
    Generates training targets for Panoptic-DeepLab.
    """
    ...
    def __call__(self, perturbed_masks, height, width, ori_h, ori_w):
        """Generates the training target.
        reference: https://github.com/mcordts/cityscapesScripts/blob/master/cityscapesscripts/preparation/createPanopticImgs.py  # noqa
        reference: https://github.com/facebookresearch/detectron2/blob/main/datasets/prepare_panoptic_fpn.py#L18  # noqa
        Args:
        ...
        Returns:
            A dictionary with fields:
                ...
                - offset_weights: Tensor, ignore region of offset prediction,
                    ...
                    ignore, 1 is has instance. Multiply this mask to loss.
        """
        # height, width = perturbed_masks[0].shape[0], perturbed_masks[0].shape[1]
        
        center = np.zeros((ori_h, ori_w), dtype=np.float32)
        center_pts = []
        offset = np.zeros((2, ori_h, ori_w), dtype=np.float32)
        y_coord, x_coord = np.meshgrid(
            np.arange(ori_h, dtype=np.float32), np.arange(ori_w, dtype=np.float32), indexing="ij"
        )
       
        for perturbed_mask in perturbed_masks:
            # find instance center
            perturbed_mask = cv2.resize(perturbed_mask, (ori_w, ori_h), interpolation=cv2.INTER_NEAREST)
            ...
            # start and end indices in default Gaussian image
            gaussian_x0, gaussian_x1 = max(0, -ul[0]), min(br[0], ori_w) - ul[0]
            gaussian_y0, gaussian_y1 = max(0, -ul[1]), min(br[1], ori_h) - ul[1]

            # start and end indices in center heatmap image
            center_x0, center_x1 = max(0, ul[0]), min(br[0], ori_w)
            center_y0, center_y1 = max(0, ul[1]), min(br[1], ori_h)
            center[center_y0:center_y1, center_x0:center_x1] = np.maximum(
                center[center_y0:center_y1, center_x0:center_x1],
                self.g[gaussian_y0:gaussian_y1, gaussian_x0:gaussian_x1],
            )
            ...
            offset[0][mask_index] = (center_y - y_coord[mask_index]) / ori_h
            offset[1][mask_index] = (center_x - x_coord[mask_index]) / ori_w
        

        offsets = np.stack([center, offset[0], offset[1]], axis=0)
        offsets = torch.as_tensor(offsets.astype(np.float32))
        offsets = torch.nn.functional.interpolate(offsets.unsqueeze(0), size=(height, width), mode='bilinear').squeeze(0)

        return dict(
            initial_pred_offset=offsets, # [3, H, W]
