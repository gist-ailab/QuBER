...
classes into one segment with semantic class 'other'.
 ...
 python tools/ours/panoptic2semantic_segmentation.py --input_json_file detectron2_datasets/UOAIS-Sim/annotations/uoais_sim_train_panoptic_perturbed.json  --categories_json_file tools/ours/panoptic_uoais_sim_categories.json --segmentations_folder detectron2_datasets/UOAIS-Sim/train --output_json_file uoais_sim_semantic_train.json
'''
...
def extract_semantic(input_json_file,
    ...
    with open(input_json_file, 'r') as f:
        d_coco = json.load(f)
    for idx in range(len(d_coco['images'])):
        del d_coco['images'][idx]['perturbed_segmentation']
    ...
    if not save_as_png:
        for idx, ann in enumerate(annotations_coco_semantic_seg):
            ann['id'] = idx
        for idx in range(len(annotations_coco_semantic_seg)):
            annotations_coco_semantic_seg[idx]['image_id'] = int(annotations_coco_semantic_seg[idx]['image_id'])
