...
python tools/ours/detection2panoptic_coco_format.py --input_json_file /SSDe/sangbeom_lee/mask-eee-rcnn/datasets/armbench/mix-object-tote/train_perturbed.json --output_json_file /SSDe/sangbeom_lee/mask-eee-rcnn/datasets/armbench/mix-object-tote/train_panoptic_perturbed.json --segmentations_folder /SSDe/sangbeom_lee/mask-eee-rcnn/datasets/armbench/train --categories_json_file detectron2_datasets/UOAIS-Sim/annotations/panoptic_uoais_sim_categories.json

python tools/ours/detection2panoptic_coco_format.py --input_json_file detectron2_datasets/UOAIS-Sim/annotations/coco_anns_uoais_sim_val_augmented_perturbed.json --output_json_file detectron2_datasets/UOAIS-Sim/annotations/uoais_sim_val_panoptic_augmented_perturbed.json --segmentations_folder detectron2_datasets/UOAIS-Sim/val --categories_json_file detectron2_datasets/UOAIS-Sim/annotations/panoptic_uoais_sim_categories.json
...
def convert_detection_to_panoptic_coco_format_single_core(
    ...
    for working_idx, img_id in enumerate(img_ids):
        ...
        file_name = file_name.replace("color", "panoptic")
        ...
        out_path  = os.path.join(segmentations_folder, file_name)
        if not os.path.exists(out_path.rsplit('/', 1)[0]):
            os.makedirs(out_path.rsplit('/', 1)[0])
...
def convert_detection_to_panoptic_coco_format(input_json_file,
    ...
    with open(input_json_file, 'r') as f:
        d_coco = json.load(f)
    for idx in range(len(annotations_coco_panoptic)):
        annotations_coco_panoptic[idx]['image_id'] = int(annotations_coco_panoptic[idx]['image_id'])
    for idx in range(len(categories_list)):
        categories_list[idx]['id'] = int(categories_list[idx]['id'])
    d_coco['annotations'] = annotations_coco_panoptic
    d_coco['categories'] = categories_list
    # for idx in range(len(d_coco['segmentation_info'])):
    #     d_coco['segmentation_info'][idx]['id'] = int(d_coco['segmentation_info'][idx]['id'])
    #     d_coco['segmentation_info'][idx]['category_id'] = int(d_coco['segmentation_info'][idx]['category_id'])
    #     d_coco['segmentation_info'][idx]['area'] = int(d_coco['segmentation_info'][idx]['area'])
    #     d_coco['segmentation_info'][idx]['iscrowd'] = int(d_coco['segmentation_info'][idx]['iscrowd'])

    
    print("Error while saving json file. Saving to json.txt")
