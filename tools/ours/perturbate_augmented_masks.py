...
gs_ratio_range = [0.0, 0.2]
merge_ratio_range = [0.0, 0.1]
delte_ratio_range = [0.0, 0.05]
split_ratio_range = [0.0, 0.1]
iou_target_range = [0.8, 1.0]
perturb_ratio = 0.7
min_mask_ratio = 0.01

coco_anno_path = os.path.join(uoais_sim_path, 'annotations', 'coco_anns_uoais_sim_{}_augmented.json'.format(split))
coco_anno = COCO(coco_anno_path)
img_ids = coco_anno.getImgIds()

purturbed_coco_anno_path = os.path.join(uoais_sim_path, 'annotations', 'coco_anns_uoais_sim_{}_augmented_perturbed.json'.format(split))
...
for img_id in tqdm(img_ids):
    ...
    gt_masks = np.array([m.decode(anno['segmentation']) for anno in annos])
    ...
    gs_masks = segment(np.array(img_resized), 0.2, 50, 100) # [height, width] with values in [0, 255]
    ...
    for idx1 in np.arange(len(perturbated_masks)):
        ...
        dilate_mask = cv2.dilate(mask1.copy(), np.ones((5, 5), np.uint8), iterations=1)
        for idx2 in np.arange(len(perturbated_masks)):
            ...
            if np.sum(dilate_mask * mask2) > 0:
                perturbated_masks[idx1] = np.logical_or(dilate_mask, mask2).astype(np.uint8) * 255
    ...
    for anno in annos:
        perturbed_anno = {
            ...
            'bbox': anno['bbox'],
            'area': int(np.sum(m.decode(anno['segmentation']))),
            'iscrowd': 0,
            'height': anno['height'],
            'width': anno['width'],
        }
