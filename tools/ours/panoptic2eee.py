...
def masks_to_fg_mask(masks, h, w):
...
def mask_to_boundary(mask, h, w, dilation_ratio=0.02):
    """
...
def masks_to_boundary(masks, h, w, dilation_ratio=0.01):
    # masks = [N, H, W], numpy array
    fg_mask = masks_to_fg_mask(masks, h, w)
    boundary = np.zeros_like(fg_mask)
    for mask in masks:
        boundary += mask_to_boundary(mask, h, w, dilation_ratio=dilation_ratio)
    boundary = boundary > 0
...
json_file = '/SSDe/sangbeom_lee/mask-eee-rcnn/datasets/armbench/mix-object-tote/test_panoptic_perturbed.json'
segmentations_folder = '/SSDe/sangbeom_lee/mask-eee-rcnn/datasets/armbench/test'
img_folder = '/SSDe/sangbeom_lee/mask-eee-rcnn/datasets/armbench/mix-object-tote/images'

json_file = '/SSDe/sangbeom_lee/mask-eee-rcnn/datasets/armbench/mix-object-tote/train_panoptic_perturbed.json'
segmentations_folder = '/SSDe/sangbeom_lee/mask-eee-rcnn/datasets/armbench/train'
...
for img_info, anno in zip(img_infos, tqdm(annos)):
    ...
    img = utils.read_image(os.path.join(img_folder, img_info["file_name"]), "RGB")
    h, w = img.shape[:2]
    pan_seg_gt = utils.read_image(os.path.join(segmentations_folder, img_info["file_name"]).replace('.jpg', '.png'), "RGB")
    ...
    gt_fg_mask = masks_to_fg_mask(gt_masks, h, w)
    gt_boundary = masks_to_boundary(gt_masks, h, w)
    perturbed_fg_mask = masks_to_fg_mask(perturbed_masks, h, w)
    perturbed_boundary = masks_to_boundary(perturbed_masks, h, w)
