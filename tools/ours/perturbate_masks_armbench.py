...
coco_path = '/SSDe/sangbeom_lee/mask-eee-rcnn/datasets/armbench/mix-object-tote'
split = 'test'
...
min_mask_ratio = 0.01
coco_anno_path = os.path.join(coco_path, '{}.json'.format(split))
coco_anno = COCO(coco_anno_path)
img_ids = coco_anno.getImgIds()

purturbed_coco_anno_path = os.path.join(coco_path, '{}_perturbed.json'.format(split))
purturbed_coco_anno_json = {
    "info": {
        "description": "ARMBENCH Dataset",
...
for img_id in tqdm(img_ids):
    ...
    # load COCO GT
    anno_ids = coco_anno.getAnnIds(imgIds=img_id, catIds=[2])
    annos = coco_anno.loadAnns(anno_ids)
    img_info = coco_anno.loadImgs(img_id)[0]
    img_path = os.path.join(coco_path, 'images', img_info['file_name'])
    img = cv2.imread(img_path)
    height, width = img.shape[:2]

    # gt_masks: [num_instances, height, width], values in {0, 1}
    gt_masks = np.array([coco_anno.annToMask(anno) for anno in annos])
    ...
    h, w = img.shape[:2]
    img_resized = cv2.resize(img, (640//6, 480//6))
    ...
    for anno in annos:
        perturbed_anno = {
            ...
            'bbox': anno['bbox'],
            'area': int(np.sum(coco_anno.annToMask(anno))),
            'iscrowd': 0,
            'height': int(height),
            'width': int(width),
        }
