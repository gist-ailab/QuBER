...
coco_path = '/SSDe/seunghyeok_back/mask-refiner/detectron2_datasets/coco'
split = 'val'
...
min_mask_ratio = 0.01
coco_anno_path = os.path.join(coco_path, 'annotations', 'instances_{}2017.json'.format(split))
coco_anno = COCO(coco_anno_path)
img_ids = coco_anno.getImgIds()

purturbed_coco_anno_path = os.path.join(coco_path, 'annotations', 'instances_{}_perturbed.json'.format(split))
...
for img_id in tqdm(img_ids):
    ...
    img_path = os.path.join(coco_path, '{}2017'.format(split), img_info['file_name'])
    ...
    h, w = img.shape[:2]
    img_resized = cv2.resize(img, (w//6, h//6))
