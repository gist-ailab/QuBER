...
def create_dataloaders(name_im_gt_list, my_transforms=[], batch_size=1, training=False):
    ...
    if training:
        ...
        # gos_dataset = OnlineDatasetUOAISSIM(transform = transforms.Compose(my_transforms))
        gos_dataset = OnlineDatasetArmBench(transform = transforms.Compose(my_transforms))
...
class OnlineDatasetUOAISSIM(Dataset):
    def __init__(self, transform=None, eval_ori_resolution=False):

        self.transform = transform
        ## combine different datasets into one
        self.root = '/SSDe/seunghyeok_back/mask-refiner/detectron2_datasets/UOAIS-Sim'
        self.img_root = os.path.join(self.root, 'train')
        ann_json = os.path.join(self.root, 'annotations/coco_anns_uoais_sim_train.json')

        self.coco = COCO(ann_json)
        self.ids = list(self.coco.anns.keys())  # list of all annotation ids in the dataset
    ...
    def __getitem__(self, idx):
        ...
        im = io.imread(os.path.join(self.root, 'train', path))
...
class OnlineDatasetArmBench(Dataset):
    def __init__(self, transform=None, eval_ori_resolution=False):

        self.transform = transform
        ## combine different datasets into one
        self.root = '/SSDe/sangbeom_lee/mask-eee-rcnn/datasets/armbench/mix-object-tote'
        ann_json = os.path.join(self.root, 'train.json')

        self.coco = COCO(ann_json)
        # self.ids = list(self.coco.anns.keys())  # list of all annotation ids in the dataset
        self.ids = self.coco.getAnnIds(catIds=[2])
        self.img_ids = list(self.coco.imgs.keys())
    ...
    def __getitem__(self, idx):
        ...
        img_id = coco.anns[ann_id]['image_id']
        path = coco.loadImgs(img_id)[0]['file_name']
        im = io.imread(os.path.join(self.root, 'images', path))
